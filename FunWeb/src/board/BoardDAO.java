...
//				BoardList.add("string"); // boardBena객체만 저장되므로 스트링형은 저장이 안된다
	...
	public List<BoardBean> getBoardList(int startRow, int pageSize, int type) {
...
//				BoardList.add("string"); // boardBena객체만 저장되므로 스트링형은 저장이 안된다
	...
	public List<BoardBean> getBoardList(int startRow, int pageSize, String search, int type) {
	...
	public int insertBoard(BoardBean bb) {
		// 답글추가:  그룹번호 re_ref(=num), 답글 들여쓰기 (re_lev)(0), 답글의 순서 (re_seq)(0)
		...
		try {
			...
			sql ="insert into board (num, pass, name, subject, content, readcount, date, file, type, re_ref, re_lev, re_seq) values(?,?,?,?,?,0,now(),?,?,?,?,?)";
...
//			ps.setInt(6, bb.getReadcount());	
			...
			ps.setInt(7, bb.getType());
			ps.setInt(8, num);	// 그룹번호 re_ref(==num)
			ps.setInt(9, 0);	// 답글 들여쓰기 (re_lev)(0) 0번쨰 레벨 (최상위)
			ps.setInt(10, 0);	// 답글의 순서 (re_seq)(0) 0번째 순서
